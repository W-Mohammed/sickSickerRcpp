...
// [[Rcpp::export]]
arma::mat ProbsV_Cpp( const arma::rowvec v_S_t,
                      const int& n_I,
                      const int& n_S,
                      const arma::mat& t_P,
                      const arma::mat& m_t_states, 
                      const std::vector<int>& v_states_from, 
                      const std::vector<int>& v_states_to,
                      const std::vector<int>& v_states_comp,
                      const std::vector<double>& v_increase_rate) {
...
// [[Rcpp::export]]
arma::colvec SampleV_Cpp( const arma::mat& m_P_t,
                          const int& n_I,
                          const int& n_S) {
...
// [[Rcpp::export]]
arma::colvec CostsV_Cpp( const arma::colvec v_S_t,
                         const arma::vec& v_Costs) {
  // v_S_t: vector of health states occupied by individuals at cycle t.
...
// [[Rcpp::export]]
arma::colvec EffsV_Cpp( const arma::colvec v_S_t,
                        const arma::vec& v_Utilities,
                        const arma::vec& v_d_Utilities,
                        const arma::mat& m_t_states,
                        const int cycle = 1 ) {
  // v_S_t: vector of health states occupied by individuals at cycle t.
  ...
  // Estimating dis-utilities based on the time spent in each state:
  // Use states indicies to assign utilities correctly:
  arma::vec v_col_Utilities = v_Utilities.elem(uv_indices) - (m_t_states * v_d_Utilities) ;
...
// v_d_Utilities numeric vector containing dis-utilities for each health state.
...
Rcpp::List MicroSimV_Cpp_4(const arma::colvec& v_S_t,
                           const arma::mat& m_t_P,
                           const arma::vec& v_Costs,
                           const arma::vec& v_Utilities,
                           const int n_I,
                           const int n_S = 4,
                           const int n_T = 30,
                           const arma::vec& v_d_Utilities = arma::vec(),
                           const double n_Cycle_length = 1,
                           const double d_dC = 0.03,
                           const double d_dE = 0.03,
                           const std::vector<int>& v_tracked_states = std::vector<int>(), 
                           const std::vector<int>& v_states_from = std::vector<int>(),
                           const std::vector<int>& v_states_to = std::vector<int>(),
                           const std::vector<int>& v_states_comp = std::vector<int>(),
                           const std::vector<double>& v_increase_rate = std::vector<double>(),
                           const int n_Seed = 1 ) {
  // Arguments:
  // v_S_t: numeric vector containing the health states occupied by the individuals at cycle t
  // m_t_P: matrix containing transition probabilities
  ...
  m_E.col(0) = EffsV_Cpp(m_M.col(0), v_Utilities,  v_d_Utilities,  m_t_states, n_Cycle_length) ;
  ...
  for(int t = 0; t < n_T; t++) {
    ...
    m_E.col(t + 1) = EffsV_Cpp(m_M.col(t + 1),  v_Utilities, v_d_Utilities, 
            m_t_states, n_Cycle_length) ;
