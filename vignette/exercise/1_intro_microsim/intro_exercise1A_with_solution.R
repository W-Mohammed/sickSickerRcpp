...
# Yes! the probabilities are in the correct order
...
rowSums(m_trans_probs)
rowSums(m_trans_probs) == 1 # runs the check for each elements in the vector
all(rowSums(m_trans_probs) == 1) # runs the check for all elements in the vector
...
v_probs[occupied_state == "H"] <- m_trans_probs["H",]
...
v_probs[occupied_state == "S1"] <- m_trans_probs["S1",]
...
# The probabilities of moving to health state "S1" from "H", "S1", "S2", & "D".
...
v_probs[occupied_state == "S2"] <- m_trans_probs["S2",]
...
# 'occupied_state' and 'm_trans_probs'
